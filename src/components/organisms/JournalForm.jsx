...
export default function JournalForm({ isEdit = false, initialData = null }) {
    ...
    useEffect(() => {
        if (isEdit && initialData) {
            ...
            setTime(initialData.time || new Date().toTimeString().split(' ')[0].slice(0, 5));
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (isEdit) {
                await axios.put(
                    `http://localhost:5000/api/journal/update/${initialData._id}`,
                    payload,
                    ...
                    <div className="relative">
                        ...
                        <div className="relative">
                            <Input
                                ...
                                required
                            />
                            <Calendar size={18} className="absolute left-4 top-3.5 text-text-muted/60 pointer-events-none" />
                        </div>
                    </div>
                    <div className="relative">
                        <label className="block text-sm font-semibold text-text-dark mb-2">
                        ...
                        <div className="relative">
                            <Input
                                ...
                                required
                            />
                            <Calendar size={18} className="absolute left-4 top-3.5 text-text-muted/60 pointer-events-none" opacity={0} /> {/* Spacer to match date layout */}
                        </div>
                    </div>
                </div>
