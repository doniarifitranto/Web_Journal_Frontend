...
export default function Sidebar({ isOpen, onClose }) {
  ...
  const fetchRecentJournals = async () => {
    try {
      ...
      setRecentJournals(response.data.journals || []);
    } catch (error) {
  ...
  const handleLogout = async () => {
    try {
      ...
      // Selalu bersihkan state lokal meski request gagal
      setUser(null);
      try {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed md:static top-0 left-0 z-50 w-64 h-screen bg-white border-r border-neutral-border flex flex-col justify-between p-6 transition-transform duration-300 ${isOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
      >
        <div>
          {/* Logo */}
          <div className="flex items-center gap-3 mb-10 pl-2">
            ...
            <span className="text-xl font-bold tracking-tight text-sage-dark">
              MoodLens
            </span>
          </div>

          {/* Main Menu */}
          <p className="text-[11px] font-bold text-text-muted/60 tracking-wider uppercase mb-3 pl-2">
            Main Menu
          </p>
            ...
            {menuItems.map((item) => {
              ...
              return (
                ...
                >
                  <Icon
                    size={18}
                    className={isActive ? "text-sage-dark" : "text-text-muted"}
                  />
                  {item.name}
                </Link>
              );
          ...
          <div className="flex items-center justify-between mb-3 pl-2 pr-1">
            <p className="text-[11px] font-bold text-text-muted/60 tracking-wider uppercase">
              Journal
            </p>
            ...
            <Link
              ...
              onClick={onClose}
              className="text-text-muted hover:text-sage-dark p-1 rounded-lg hover:bg-mood-bg transition-colors"
            >
              <Plus size={16} />
            </Link>
          </div>
          ...
          <div className="space-y-1">
            {recentJournals.length > 0 ? (
              ...
              <p className="text-[11px] text-text-muted/50 pl-2 italic">
                No entries yet
              </p>
            )}
          </div>
        ...
        <div>
          ...
          <div className="space-y-1">
            <Link
              ...
              className="flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium text-text-muted hover:bg-mood-bg hover:text-sage-dark"
            >
              <Settings size={18} /> Settings
            </Link>
            <button
              onClick={handleLogout}
              className="w-full flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium text-status-burnout hover:bg-status-burnout/10 transition-colors"
            >
