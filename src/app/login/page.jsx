    ...
    try {
      ...
      setErrorMsg(
        error.response?.data?.message ||
          "Credential not valid or server went wrong!",
      );
    } finally {
      setIsLoading(false);
    }
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex items-center justify-between mt-2">
              <label className="flex items-center gap-2 text-sm text-text-muted cursor-pointer">
                <input
                  type="checkbox"
                  className="rounded border-neutral-border text-sage-dark focus:ring-sage-dark"
                />
                Remember me
              </label>
              <a
                href="#"
                className="text-sm text-sage-dark hover:text-sage-700 hover:underline font-medium transition-colors"
              >
                Forgot password?
              </a>
            </div>
